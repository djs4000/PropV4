...
char pendingKeypadDigit = '\0';
bool keypadDigitReady = false;
...
InputSnapshot readInputSnapshot() {
  ...
  snapshot.bothButtonsPressed = debouncedButtons;

  // Debounce keypad entries to surface a single digit event.
...
#endif
    keypadDigitReady = (debouncedKey >= '0' && debouncedKey <= '9');
    pendingKeypadDigit = keypadDigitReady ? debouncedKey : '\0';
  }

  snapshot.hasKeypadDigit = keypadDigitReady;
  snapshot.keypadDigit = pendingKeypadDigit;
  keypadDigitReady = false;
  pendingKeypadDigit = '\0';

  snapshot.irConfirmationReceived = irConfirmationPending;
